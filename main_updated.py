import asyncio
import threading
import os
import time
from flask import Flask, jsonify
from admin_bot import create_admin_app
from friend_bot import create_friend_app
from config import check_config, TEST_VIDEO_URL, ADMIN_BOT_TOKEN
from sheets import log_action
from video_handler import process_test_video
from automation1 import start_automation1

flask_app = Flask(__name__)

class BotState:
    admin_app = None
    friend_app = None
    loop = None

state = BotState()

@flask_app.route("/")
def health():
    return "🚀 LAMAZAT CANVAS — SERVER LIVE! (Automation 1 Running)"

@flask_app.route("/test")
def trigger_test():
    if not state.admin_app or not state.loop:
        return jsonify({"status": "error", "message": "Bots starting... wait 10s and refresh."})
    try:
        state.loop.call_soon_threadsafe(
            lambda: asyncio.run_coroutine_threadsafe(
                process_test_video(state.admin_app.bot, state.friend_app.bot, TEST_VIDEO_URL),
                state.loop
            )
        )
        return jsonify({"status": "success", "message": "Test triggered! Check Telegram."})
    except Exception as e:
        return jsonify({"status": "error", "message": str(e)})

@flask_app.route("/automation1/status")
def auto1_status():
    """Automation 1 ka current status check"""
    try:
        from sheets import get_early_pool_count, get_automation_flag
        count = get_early_pool_count()
        flag  = get_automation_flag("Automation1")
        return jsonify({
            "status": flag.get("status") if flag else "Unknown",
            "pool": count
        })
    except Exception as e:
        return jsonify({"error": str(e)})

async def start_bots_async():
    print("🤖 Starting Bots...")
    state.admin_app = create_admin_app()
    state.friend_app = create_friend_app()
    state.loop = asyncio.get_running_loop()

    await state.admin_app.initialize()
    await state.friend_app.initialize()
    await state.admin_app.start()
    await state.friend_app.start()

    await state.admin_app.updater.start_polling()
    await state.friend_app.updater.start_polling()

    print("✅ BOTS ONLINE!")
    while True:
        await asyncio.sleep(3600)

def bot_worker():
    loop = asyncio.new_event_loop()
    asyncio.set_event_loop(loop)
    loop.run_until_complete(start_bots_async())

def automation1_worker():
    """Automation 1 — alag thread mein chalega"""
    print("🔍 Starting Automation 1 — Trending Product Detection...")
    start_automation1(ADMIN_BOT_TOKEN)

# Bots start karo
threading.Thread(target=bot_worker, daemon=True).start()

# Automation 1 start karo (10 second delay — bots pehle ready ho jaayein)
def delayed_auto1():
    time.sleep(10)
    automation1_worker()

threading.Thread(target=delayed_auto1, daemon=True).start()

if __name__ == "__main__":
    port = int(os.environ.get("PORT", 10000))
    flask_app.run(host="0.0.0.0", port=port)
