...
async def extract_products_from_page(page):
    ...
    # Scroll karke lazy-load trigger karo
    for _ in range(5):
    ...
    # ✅ FIX: selector ko Python f-string ke andar inject karne ki bajay
    # JS ko argument ke roop mein pass karo — single quotes conflict nahi karenge
    js_data = await page.evaluate(
        ...
        (selector) => {
            ...
            cards.forEach((card, i) => {
                ...
                const nameEl = card.querySelector(
                    'h1,h2,h3,h4,[class*="title"],[class*="name"],[class*="Title"],[class*="Name"],p.name,span.name'
                );
                ...
                const priceEl = card.querySelector(
                    '[class*="price"],[class*="Price"],[class*="cost"],span.amount,.amount'
                );
        ...
        """,
        used_sel   # ← selector Python se JS ko argument ke roop mein pass ho raha hai
    )
