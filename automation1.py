...
def fetch_eprolo_products(min_signal="Medium"):
    """Eprolo Products tab se products lo"""
    try:
        sheet = get_sheet()
        if not sheet:
            return []
        ...
        # Signal filter — sab levels allowed (test mode)
        all_levels = ["Strong", "Medium", "Weak", "Very Weak"]
        if min_signal == "Strong":
            allowed = ["Strong"]
        elif min_signal == "Medium":
            allowed = ["Strong", "Medium"]
        elif min_signal == "Weak":
            allowed = ["Strong", "Medium", "Weak"]
        else:
            allowed = all_levels  # Very Weak bhi chalega

        result = [r for r in records if r.get("signal_strength") in allowed
                  and r.get("status") == "Active"]

        log_action("Eprolo Fetch", f"{len(result)} products found (min_signal={min_signal})", "Success")
        return result
...
async def run_session(admin_bot_token, products_to_check=None):
    ...
    if not products_to_check:
        products_to_check = fetch_eprolo_products(min_signal="Very Weak")
...
async def run_product_test(admin_bot_token):
    ...
    eprolo_products = fetch_eprolo_products(min_signal="Very Weak")
    ep = None
