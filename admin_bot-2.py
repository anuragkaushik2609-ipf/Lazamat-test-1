...
from config import ADMIN_BOT_TOKEN, ADMIN_CHAT_ID, FRIEND_CHAT_ID
from sheets import (
...
# ─────────────────────────────────────────
# ADMIN → FRIEND BOT FORWARD
# Admin MP4 ya URL bheje → automatically Friend Bot par forward ho
# Sirf MP4 aur URL — kuch nahi
# ─────────────────────────────────────────

def _is_video_url(text: str) -> bool:
    """Check karo ki text ek video URL hai ya nahi."""
    if not text:
        return False
    text = text.strip()
    # Common video platforms + direct links
    video_domains = [
...
async def handle_admin_media(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Admin se MP4 ya URL receive karo → Friend Bot par forward karo.
    Sirf admin ka message process hoga.
    """
    if not is_admin(update):
        return
    ...
    if message.video or (message.document and message.document.mime_type == "video/mp4"):
        ...
        try:
            # Friend Bot par forward karo
            await context.bot.send_video(
                chat_id=FRIEND_CHAT_ID,
                video=media.file_id,
                caption=(
                    f"📤 *Admin ne video bheja*\n\n"
                    f"{caption}\n\n"
                    "🔑 *Agar unique code hai toh saath mein likho*\n"
                    "Example: `LMZT-A3K9`"
                ) if not caption else (
                    f"📤 *Admin se video:*\n\n"
                    f"{caption}"
            ...
            forwarded = True
            log_action("AdminBot", "MP4 forwarded to Friend Bot", "Success")

            await message.reply_text(
                "✅ *Video Friend Bot par forward ho gayi!*",
                parse_mode="Markdown"
            )

        except Exception as e:
            print(f"❌ MP4 forward error: {e}")
            await message.reply_text(f"❌ Forward nahi hua: {str(e)}")
            log_action("AdminBot", f"MP4 forward failed: {str(e)}", "Error")

    # ── Case 2: URL (text message mein) ──
    elif message.text and _is_video_url(message.text):
        ...
        try:
            ...
            forwarded = True
            log_action("AdminBot", f"URL forwarded to Friend Bot: {url[:50]}", "Success")

            await message.reply_text(
                "✅ *URL Friend Bot par forward ho gayi!*",
                parse_mode="Markdown"
            )

        except Exception as e:
            print(f"❌ URL forward error: {e}")
            await message.reply_text(f"❌ Forward nahi hua: {str(e)}")
            log_action("AdminBot", f"URL forward failed: {str(e)}", "Error")

    # ── Kuch aur aaya — ignore (custom_time handler ke liye pass hoga) ──

async def handle_admin_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """
    Admin ke text messages handle karo:
    - Agar URL hai → Friend Bot par forward
    - Agar custom time wait kar raha tha → process karo
    - Baaki sab → ignore
    """
    if not is_admin(update):
        return

    # Custom time pehle check karo
    if context.user_data.get("waiting_custom_time"):
        await handle_custom_time(update, context)
        return

    # URL check karo
    if update.message.text and _is_video_url(update.message.text):
        await handle_admin_media(update, context)
        return

    # Kuch aur aaya — ignore silently
...
def create_admin_app():
    ...
    # ── MP4 forward handler (priority 1 — pehle check hoga) ──
    ...
    app.add_handler(MessageHandler(
        ...
        handle_admin_text  # Naya combined handler — neeche define hai
    ), group=1)

    log_action("Admin Bot", "Bot initialized", "Success")
    return app
